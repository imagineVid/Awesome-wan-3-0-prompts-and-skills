/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地视频提示词指纹与 Wan 3.0 模型证据
 * [OUTPUT]: 对外提供候选评分、硬门槛、文本与媒体去重及已发布数据校验能力
 * [POS]: scripts/utils 的社区内容质量策略，是搜索候选进入公开仓库前的唯一规则层
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  if (
    !["wan-3.0", "wan-video-3.0"].includes(
      prompt.sourceMeta?.model_evidence || ""
    )
  ) {
    hardFailures.push("missing verified Wan 3.0 model evidence");
  }
  ...
  if (prompt.videoCategories?.workflows?.length !== 1) {
    hardFailures.push("missing primary workflow category");
  }
  ...
  if (prompt.videoCategories?.workflows?.length === 1) score += 5;
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasPromptSignal =
    /(?:^|\n)\s*(?:video\s+)?prompt\s*(?:[:：👇]|below|in replies)/im.test(text) ||
    /(?:camera|shot|dialogue|audio|duration|image[-\s]to[-\s]video)\s*:/i.test(text);
  const relevanceText = `${text}\n${candidate.quoted_tweet?.text || ""}`;
  const hasWanEvidence = /\bwan(?:[-\s]*video)?[-\s]*3(?:\.0)?\b|\bwan3\b/i.test(
    relevanceText
  );
  const namesDifferentVideoModel = /\b(?:veo|kling|seedance|sora|hailuo|runway|luma)\b/i.test(
    relevanceText
  ) && !hasWanEvidence;
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasWanEvidence) {
    reasons.push("missing explicit Wan 3.0 evidence; manual review required");
  ...
  if (namesDifferentVideoModel) {
    reasons.push("explicitly targets a different video model");
    decision = "reject";
  }
